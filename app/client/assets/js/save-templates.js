const saveTemplatesForm = document.getElementById('create-template-form');
const messageMod = document.getElementById('modal-message');
  ...
  if (type === 'success') {
    messageMod.innerHTML = `
                <div id="toast-success" class="flex items-center w-full mx-auto max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800 " role="alert">
                    ...
                    <div id="message" class="ml-3 text-sm font-normal">${message}</div>
...
saveTemplatesForm.addEventListener('htmx:afterRequest', function (event) {
  const response = JSON.parse(event.detail.xhr.response);
  const message = response['message'];
  const success_message = 'Template saved successfully';
